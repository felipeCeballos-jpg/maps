...
const getImageSources = (langauge) => ({
  translate: LOCALIZEDIMAGES[langauge]?.images || [],
  /* design: originalImages, */
  footer: langauge === 'ru' ? footerRuTranslateImgs : footerEnTranslateImgs,
});

export async function updateImages(langauge) {
  if (!langauge) {
  ...
  try {
    // Get images sources
    const imageSources = getImageSources(langauge);
